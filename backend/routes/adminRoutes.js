...
router.delete("/reject-user/:id", async (req, res, next) => {
  try {
...
// PUT /force-update-user/:id
router.put("/force-update-user/:id", async (req, res, next) => {
  try {
    const mongoose = require('mongoose');
    const { name, email, phone, dob, gender, oldEmail } = req.body;
    let user = null;
    ...
    if (!user && oldEmail) {
      ...
      user = await User.findOne({ email: { $regex: new RegExp("^" + oldEmail.trim() + "$", "i") } });
    }
    if (!user && email) {
    ...
    if (!user) {
        console.log(`[FORCE-UPDATE] User not found. tried lookup by ID: ${req.params.id}, oldEmail: ${oldEmail}, newEmail: ${email}`);
    ...
    console.log(`[FORCE-UPDATE] Updating user: ${user.email} with data:`, updateData);
    
    // Use findByIdAndUpdate for a more "forceful" update that can bypass some record-level hurdles
    const updatedUser = await User.findByIdAndUpdate(user._id, { $set: updateData }, { new: true, runValidators: false });

    console.log(`[FORCE-UPDATE] Saved successfully`);
    res.json({ message: "User forcefully updated", user: updatedUser });
  } catch (error) {
    console.error("[FORCE-UPDATE] Catch block error:", error);
